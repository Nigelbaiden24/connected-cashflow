...
import { Loader2, Bitcoin, Globe } from "lucide-react";
...
export function CryptoReportScraper() {
  ...
  const handleScrape = async () => {
    ...
    try {
      ...
      setResult(md);
      toast.success("Crypto report scraped");
    } catch (e: any) {
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle>Crypto Report Scraper</CardTitle>
              <CardDescription>Scrape digital asset research and on-chain analysis from any URL</CardDescription>
            </div>
          </div>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex gap-2">
            <Input
              placeholder="https://example.com/crypto-report"
              value={url}
