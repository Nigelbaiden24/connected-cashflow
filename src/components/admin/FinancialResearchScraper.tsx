    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        ...
        <TabsContent value="platforms" className="space-y-4">
          {/* Investment Categories — curated sources per asset class */}
            ...
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between flex-wrap gap-2">
                <div>
                  ...
                  <CardDescription>
                    Sweep curated sources for each asset class — extracts news, data, and opportunity candidates
                  </CardDescription>
                </div>
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" onClick={() => setSelectedCategories(INVESTMENT_CATEGORIES.map(c => c.id))}>
                    Select All
                  </Button>
                  ...
                  <Button
                    ...
                    disabled={isScrapingInProgress || selectedCategories.length === 0}
                    className="gap-2"
                  >
                    {isScrapingInProgress ? <Loader2 className="h-4 w-4 animate-spin" /> : <Zap className="h-4 w-4" />}
        ...
        <TabsContent value="report" className="space-y-4">
          {!aiReport && !isGeneratingReport ? (
            ...
            <div className="space-y-4">
              ...
              {aiReport.opportunityCandidates && aiReport.opportunityCandidates.length > 0 && (
                <Card className="border-primary/30">
                  <CardHeader>
                    <div className="flex items-start justify-between gap-3 flex-wrap">
                      <div>
                        <CardTitle className="flex items-center gap-2">
                          ...
                          Opportunity Candidates ({aiReport.opportunityCandidates.length})
                        </CardTitle>
                        <CardDescription>AI-extracted investable ideas — auto-upload to live opportunities</CardDescription>
                      </div>
                      <Button
                        size="sm"
                        onClick={promoteAllOpportunities}
                        disabled={bulkPromoting || promotedIdx.size >= aiReport.opportunityCandidates.length}
                        className="gap-2"
                      >
                        {bulkPromoting ? <Loader2 className="h-4 w-4 animate-spin" /> : <CheckCircle2 className="h-4 w-4" />}
                        {bulkPromoting ? "Uploading..." : `Auto-upload all (${aiReport.opportunityCandidates.length - promotedIdx.size})`}
                      </Button>
                    </div>
                  </CardHeader>
                  <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {aiReport.opportunityCandidates.map((opp, i) => {
                        const isPromoted = promotedIdx.has(i);
                        const isPromoting = promotingIdx === i;
                        return (
                          <div key={i} className="p-3 rounded-lg border bg-muted/30 space-y-2">
                            <div className="flex items-start justify-between gap-2">
                              <h4 className="font-semibold text-sm">{opp.title}</h4>
                              {opp.confidence && (
                                <Badge variant={opp.confidence === 'high' ? 'default' : opp.confidence === 'medium' ? 'secondary' : 'outline'} className="text-xs shrink-0">
                                  {opp.confidence}
                                </Badge>
                              )}
                            </div>
                            {opp.category && <Badge variant="outline" className="text-xs">{opp.category}</Badge>}
                            {opp.asset && <p className="text-xs font-mono text-muted-foreground">{opp.asset}</p>}
                            {opp.thesis && <p className="text-xs text-muted-foreground">{opp.thesis}</p>}
                            {opp.key_data && <p className="text-xs font-medium">{opp.key_data}</p>}
                            <div className="flex items-center justify-between gap-2 pt-1">
                              {opp.source_url ? (
                                <a href={opp.source_url} target="_blank" rel="noopener noreferrer" className="text-xs text-primary hover:underline flex items-center gap-1">
                              ...
                              <Button
                                ...
                                disabled={isPromoted || isPromoting || bulkPromoting}
                                onClick={() => promoteOpportunity(opp, i)}
                                className="h-7 text-xs gap-1.5"
                              >
                                {isPromoting ? <Loader2 className="h-3 w-3 animate-spin" /> :
                                  isPromoted ? <CheckCircle2 className="h-3 w-3" /> : <Zap className="h-3 w-3" />}
                                {isPromoted ? "Uploaded" : isPromoting ? "Uploading..." : "Auto-upload"}
                              </Button>
                            </div>
                          </div>
                        );
