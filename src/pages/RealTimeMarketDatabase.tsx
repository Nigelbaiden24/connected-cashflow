        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-zinc-400 mt-2">
              Live prices from CoinGecko & Alpha Vantage APIs • {stockTotal}+ Stocks • {cryptoTotal}+ Cryptocurrencies
            </p>
          </div>
        ...
        <div className="grid grid-cols-4 gap-4 mb-6">
          <Card className="bg-gradient-to-br from-blue-500/10 to-blue-600/5 border-blue-500/20">
            <CardContent className="p-4">
              ...
              <p className="text-2xl font-bold text-white">{stockTotal.toLocaleString()}</p>
            </CardContent>
          </Card>
          <Card className="bg-gradient-to-br from-amber-500/10 to-amber-600/5 border-amber-500/20">
            <CardContent className="p-4">
              <p className="text-xs text-amber-400 font-medium uppercase">Total Crypto</p>
              <p className="text-2xl font-bold text-white">{cryptoTotal.toLocaleString()}</p>
            </CardContent>
          </Card>
        ...
        <TabsContent value="crypto" className="mt-0">
          <Card className="bg-zinc-900/50 border-white/10">
            <CardHeader>
              ...
              <CardDescription>Real-time data from CoinGecko API • Showing page {cryptoPage} of {cryptoTotalPages}</CardDescription>
            </CardHeader>
            <CardContent>
              {cryptoLoading ? (
                ...
                <>
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {filteredCrypto.map((coin, index) => (
                          ...
                          >
                            <TableCell className="text-zinc-500">{coin.marketCapRank || (cryptoPage - 1) * cryptoPerPage + index + 1}</TableCell>
                            <TableCell>
                              <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center justify-between mt-4 pt-4 border-t border-white/10">
                    <p className="text-sm text-zinc-400">
                      Showing {(cryptoPage - 1) * cryptoPerPage + 1} - {Math.min(cryptoPage * cryptoPerPage, cryptoTotal)} of {cryptoTotal}
                    </p>
                        ...
                        {[...Array(Math.min(5, cryptoTotalPages))].map((_, i) => {
                          ...
                          return (
                            <Button
                              ...
                              className={pageNum === cryptoPage ? "bg-amber-600" : "border-white/10 text-zinc-300"}
                            >
                              {pageNum}
                            </Button>
                          );
        ...
        <TabsContent value="stocks" className="mt-0">
          <Card className="bg-zinc-900/50 border-white/10">
            <CardHeader>
              ...
              <CardDescription>Real-time data from Alpha Vantage API • Showing page {stockPage} of {stockTotalPages}</CardDescription>
            </CardHeader>
            <CardContent>
              {stockLoading ? (
                <div className="space-y-4">
                ...
                <>
                  ...
                  <div className="flex items-center justify-between mt-4 pt-4 border-t border-white/10">
                    <p className="text-sm text-zinc-400">
                      Showing {(stockPage - 1) * stockPerPage + 1} - {Math.min(stockPage * stockPerPage, stockTotal)} of {stockTotal}
                    </p>
                    <div className="flex items-center gap-2">
                      <Button
                        variant="outline"
                        ...
                        {[...Array(Math.min(5, stockTotalPages))].map((_, i) => {
                          ...
                          return (
                            <Button
                              ...
                              className={pageNum === stockPage ? "bg-blue-600" : "border-white/10 text-zinc-300"}
                            >
                              {pageNum}
                            </Button>
                          );
          ...
          <DialogContent className="max-w-3xl bg-zinc-900 border-white/10 text-white">
            ...
            <div className="grid grid-cols-3 gap-4 mt-4">
              <Card className="bg-zinc-800/50 border-white/10">
                <CardContent className="p-4">
                  <p className="text-xs text-zinc-500 uppercase">Current Price</p>
                  <p className="text-xl font-bold">{formatCurrency(selectedCrypto.currentPrice)}</p>
                </CardContent>
              </Card>
              <Card className="bg-zinc-800/50 border-white/10">
                <CardContent className="p-4">
                  <p className="text-xs text-zinc-500 uppercase">24h Change</p>
              ...
              <Card className="bg-zinc-800/50 border-white/10">
                <CardContent className="p-4">
                  ...
                  <p className="text-xl font-bold">{formatMarketCap(selectedCrypto.marketCap)}</p>
                </CardContent>
              </Card>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 mt-4">
              <div className="space-y-2">
                <div className="flex justify-between py-2 border-b border-white/10">
                  ...
                  <span>{formatCurrency(selectedCrypto.high24h)}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-white/10">
                  <span className="text-zinc-400">24h Low</span>
                  <span>{formatCurrency(selectedCrypto.low24h)}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-white/10">
                  <span className="text-zinc-400">All-Time High</span>
                  <span>{formatCurrency(selectedCrypto.ath)}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-white/10">
                  <span className="text-zinc-400">All-Time Low</span>
                  <span>{formatCurrency(selectedCrypto.atl)}</span>
                </div>
              </div>
              <div className="space-y-2">
                <div className="flex justify-between py-2 border-b border-white/10">
                  <span className="text-zinc-400">Volume (24h)</span>
                  <span>{formatMarketCap(selectedCrypto.volume24h)}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-white/10">
                  <span className="text-zinc-400">Circulating Supply</span>
                  <span>{selectedCrypto.circulatingSupply?.toLocaleString() || '-'}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-white/10">
                  <span className="text-zinc-400">Total Supply</span>
                  <span>{selectedCrypto.totalSupply?.toLocaleString() || '-'}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-white/10">
                  <span className="text-zinc-400">Max Supply</span>
                  <span>{selectedCrypto.maxSupply?.toLocaleString() || '∞'}</span>
                </div>
              </div>
            </div>

            <div className="grid grid-cols-5 gap-2 mt-4">
              <Card className={`${(selectedCrypto.priceChange1h || 0) >= 0 ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-red-500/10 border-red-500/20'}`}>
                <CardContent className="p-3 text-center">
                  <p className="text-xs text-zinc-400">1h</p>
                  <p className={`font-bold ${(selectedCrypto.priceChange1h || 0) >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                    {formatPercentage(selectedCrypto.priceChange1h)}
                  </p>
                </CardContent>
              </Card>
              <Card className={`${(selectedCrypto.priceChange24h || 0) >= 0 ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-red-500/10 border-red-500/20'}`}>
                <CardContent className="p-3 text-center">
                  <p className="text-xs text-zinc-400">24h</p>
                  <p className={`font-bold ${(selectedCrypto.priceChange24h || 0) >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                    {formatPercentage(selectedCrypto.priceChange24h)}
                  </p>
                </CardContent>
              </Card>
              <Card className={`${(selectedCrypto.priceChange7d || 0) >= 0 ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-red-500/10 border-red-500/20'}`}>
                <CardContent className="p-3 text-center">
                  <p className="text-xs text-zinc-400">7d</p>
                  <p className={`font-bold ${(selectedCrypto.priceChange7d || 0) >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                    {formatPercentage(selectedCrypto.priceChange7d)}
                  </p>
                </CardContent>
              </Card>
              <Card className={`${(selectedCrypto.priceChange30d || 0) >= 0 ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-red-500/10 border-red-500/20'}`}>
                <CardContent className="p-3 text-center">
                  <p className="text-xs text-zinc-400">30d</p>
                  <p className={`font-bold ${(selectedCrypto.priceChange30d || 0) >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                    {formatPercentage(selectedCrypto.priceChange30d)}
                  </p>
                </CardContent>
              </Card>
              <Card className={`${(selectedCrypto.priceChange1y || 0) >= 0 ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-red-500/10 border-red-500/20'}`}>
                <CardContent className="p-3 text-center">
                  <p className="text-xs text-zinc-400">1y</p>
                  <p className={`font-bold ${(selectedCrypto.priceChange1y || 0) >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
      ...
      {selectedStock && (
        <Dialog open={!!selectedStock} onOpenChange={() => setSelectedStock(null)}>
          <DialogContent className="max-w-3xl bg-zinc-900 border-white/10 text-white">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-3 text-2xl">
                <Badge variant="outline" className="font-mono text-primary border-primary/30 text-lg px-3 py-1">
            ...
            <div className="grid grid-cols-3 gap-4 mt-4">
              ...
              <Card className="bg-zinc-800/50 border-white/10">
                <CardContent className="p-4">
                  <p className="text-xs text-zinc-500 uppercase">Change</p>
            ...
            <div className="grid grid-cols-2 gap-4 mt-4">
              <div className="space-y-2">
                <div className="flex justify-between py-2 border-b border-white/10">
                  ...
                  <span>{formatCurrency(selectedStock.open)}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-white/10">
                  <span className="text-zinc-400">Previous Close</span>
                  <span>{formatCurrency(selectedStock.previousClose)}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-white/10">
                  <span className="text-zinc-400">Day High</span>
                  <span>{formatCurrency(selectedStock.high)}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-white/10">
                  <span className="text-zinc-400">Day Low</span>
                  <span>{formatCurrency(selectedStock.low)}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-white/10">
                  <span className="text-zinc-400">52 Week High</span>
                  <span>{formatCurrency(selectedStock.fiftyTwoWeekHigh)}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-white/10">
                  <span className="text-zinc-400">52 Week Low</span>
                  <span>{formatCurrency(selectedStock.fiftyTwoWeekLow)}</span>
                </div>
              </div>
              <div className="space-y-2">
                <div className="flex justify-between py-2 border-b border-white/10">
                  <span className="text-zinc-400">P/E Ratio</span>
                  <span>{selectedStock.peRatio?.toFixed(2) || '-'}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-white/10">
                  <span className="text-zinc-400">Forward P/E</span>
                  <span>{selectedStock.forwardPE?.toFixed(2) || '-'}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-white/10">
                  <span className="text-zinc-400">EPS</span>
                  <span>{selectedStock.eps ? formatCurrency(selectedStock.eps) : '-'}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-white/10">
                  <span className="text-zinc-400">Dividend Yield</span>
                  <span>{selectedStock.dividendYield ? `${selectedStock.dividendYield.toFixed(2)}%` : '-'}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-white/10">
                  <span className="text-zinc-400">Beta</span>
                  <span>{selectedStock.beta?.toFixed(2) || '-'}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-white/10">
            ...
            <div className="grid grid-cols-4 gap-4 mt-4">
              <Card className="bg-zinc-800/50 border-white/10">
                <CardContent className="p-3 text-center">
                  <p className="text-xs text-zinc-400">50-Day MA</p>
                  <p className="font-bold">{formatCurrency(selectedStock.fiftyDayMA)}</p>
                </CardContent>
              </Card>
              <Card className="bg-zinc-800/50 border-white/10">
                <CardContent className="p-3 text-center">
                  <p className="text-xs text-zinc-400">200-Day MA</p>
                  <p className="font-bold">{formatCurrency(selectedStock.twoHundredDayMA)}</p>
                </CardContent>
              </Card>
              <Card className="bg-zinc-800/50 border-white/10">
                <CardContent className="p-3 text-center">
                  <p className="text-xs text-zinc-400">Analyst Target</p>
                  <p className="font-bold">{formatCurrency(selectedStock.analystTargetPrice)}</p>
                </CardContent>
              </Card>
              <Card className="bg-zinc-800/50 border-white/10">
                <CardContent className="p-3 text-center">
                  <p className="text-xs text-zinc-400">Price/Book</p>
                  <p className="font-bold">{selectedStock.priceToBook?.toFixed(2) || '-'}</p>
                </CardContent>
              </Card>
            </div>
          </DialogContent>
