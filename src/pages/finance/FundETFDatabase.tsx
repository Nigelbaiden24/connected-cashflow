...
import { Database, Search, Scale, Sparkles, TrendingUp, DollarSign, BarChart3, Globe, Download, Bookmark, Star, Building2, Landmark } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20">
      ...
      <div className="relative overflow-hidden border-b border-border/50 bg-gradient-to-r from-primary/5 via-primary/10 to-chart-2/5">
        ...
        <div className="relative p-6 md:p-8">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Enterprise research platform for UK IFAs • {fundDatabase.length.toLocaleString()} funds • {propertyFundCount} property funds
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-6 gap-3 mt-6">
            ...
            <Card className="bg-background/60 backdrop-blur-sm border-border/50 hover:border-emerald-500/30 transition-colors">
              <CardContent className="p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs text-muted-foreground">Property Funds</p>
                    <p className="text-lg font-bold">{propertyFundCount}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent className="p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-bold text-blue-500">+{avgReturn}%</p>
                  </div>
                </div>
              </CardContent>
            </Card>
      ...
      <div className="p-6 space-y-6">
        ...
        {/* Fund Category Toggle */}
        <div className="flex items-center gap-2 p-1 bg-muted/50 backdrop-blur-sm rounded-lg w-fit">
          <Button
            variant={fundCategory === 'all' ? 'default' : 'ghost'}
            size="sm"
            onClick={() => setFundCategory('all')}
            className={fundCategory === 'all' ? 'shadow-sm' : ''}
          >
            <Database className="h-4 w-4 mr-2" />
            All Funds
          </Button>
          <Button
            variant={fundCategory === 'standard' ? 'default' : 'ghost'}
            size="sm"
            onClick={() => setFundCategory('standard')}
            className={fundCategory === 'standard' ? 'shadow-sm bg-blue-600 hover:bg-blue-700' : ''}
          >
            <Landmark className="h-4 w-4 mr-2" />
            Standard Funds
          </Button>
          <Button
            variant={fundCategory === 'property' ? 'default' : 'ghost'}
            size="sm"
            onClick={() => setFundCategory('property')}
            className={fundCategory === 'property' ? 'shadow-sm bg-emerald-600 hover:bg-emerald-700' : ''}
          >
            <Building2 className="h-4 w-4 mr-2" />
            Property Funds
            <Badge variant="secondary" className="ml-2 bg-emerald-500/20 text-emerald-300 border-0">
              {propertyFundCount}
            </Badge>
          </Button>
        </div>

        {/* Property Funds Notice */}
          ...
          <Card className="bg-gradient-to-r from-emerald-500/10 to-emerald-600/5 border-emerald-500/20">
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-semibold text-emerald-400">Property Fund Database</h3>
                  <p className="text-sm text-muted-foreground">
                    Real estate investment funds including UK Direct Property, REITs, and Global Property Securities. 
                    These funds offer exposure to commercial and residential property markets.
                  </p>
                </div>
              </div>
            </CardContent>
