-- Keep promoted market commentary targeted to the selected frontend platform.
ALTER TABLE public.market_commentary
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'market_commentary_platform_check'
  ) THEN
    ALTER TABLE public.market_commentary
      ADD CONSTRAINT market_commentary_platform_check
      CHECK (platform IS NULL OR platform IN ('finance','investor','both'));
  END IF;
END $$;
...
-- Make the generic scrape promotion function preserve richer image fields and target the selected platform consistently.
...
BEGIN
  ...
  ELSE
    v_target := COALESCE(NULLIF(itm.target_table, ''), 'opportunity_products');
  END IF;

  v_stored_platform := COALESCE(NULLIF(_platform, ''), NULLIF(itm.target_platform, ''), 'both');
  ...
  v_currency := COALESCE(NULLIF(enriched->>'currency',''), NULLIF(enriched->>'price_currency',''),
    CASE WHEN v_price_raw LIKE '%£%' THEN 'GBP'
         WHEN v_price_raw LIKE '%$%' THEN 'USD'
         WHEN v_price_raw LIKE '%€%' THEN 'EUR'
         ELSE 'GBP' END);

  v_image := COALESCE(
    NULLIF(enriched->>'image_url',''), NULLIF(enriched->>'thumbnail_url',''),
  ...
  IF v_image IS NULL AND jsonb_typeof(enriched->'images') = 'array' THEN
    SELECT NULLIF(value, '') INTO v_image
    FROM jsonb_array_elements_text(enriched->'images') AS value
    WHERE value ~* '^https?://'
    LIMIT 1;
  END IF;
  ...
  IF v_image IS NULL AND jsonb_typeof(enriched->'gallery_images') = 'array' THEN
    SELECT NULLIF(value, '') INTO v_image
    FROM jsonb_array_elements_text(enriched->'gallery_images') AS value
    WHERE value ~* '^https?://'
    LIMIT 1;
  END IF;
  ...
  IF v_gallery IS NULL OR array_length(v_gallery, 1) IS NULL THEN
  ...
  ELSIF v_target = 'opportunity_products' THEN
    ...
    INSERT INTO public.opportunity_products (
      ...
      enriched->>'provenance', enriched->>'condition', 'active', false, v_platform, itm.source
    ) RETURNING id INTO new_id;
  ...
  ELSIF v_target = 'intel_events' THEN
    INSERT INTO public.intel_events (
      ...
      enriched->>'event_subtype', itm.title, itm.summary, itm.source_url,
      COALESCE(enriched->>'signal_tier', 'medium'),
      COALESCE(enriched->>'confidence', 'medium'),
      v_score, enriched, enriched, 'verified'
    ) RETURNING id INTO new_id;
  ...
  SET status = 'promoted', reviewed_at = now(), reviewed_by = auth.uid(),
      promoted_id = new_id, target_table = v_target,
      target_platform = v_stored_platform
  WHERE id = _item_id;

  RETURN jsonb_build_object('ok', true, 'promoted_id', new_id, 'target_table', v_target, 'platform', v_stored_platform, 'image_url', v_image);
...
GRANT EXECUTE ON FUNCTION public.approve_pending_item(uuid, text, text) TO authenticated;