...
-- Allow authenticated users to read pending pipeline items (so users can review/approve)
DROP POLICY IF EXISTS "Authenticated read pipeline pending" ON public.pipeline_pending_items;
CREATE POLICY "Authenticated read pipeline pending"
  ON public.pipeline_pending_items FOR SELECT
  TO authenticated
  USING (true);

-- Allow authenticated users to read intel events
DROP POLICY IF EXISTS "Authenticated read intel events" ON public.intel_events;
CREATE POLICY "Authenticated read intel events"
  ON public.intel_events FOR SELECT
  TO authenticated
  USING (true);

-- Update approve function to allow authenticated users (not just admins)
...
BEGIN
  ...
  v_image := COALESCE(NULLIF(enriched->>'image_url',''), NULLIF(enriched->>'thumbnail_url',''),
  ...
  ELSIF v_target = 'opportunity_products' THEN
    ...
    INSERT INTO public.opportunity_products (
      ...
      provenance, condition, status, featured, platform
    ) VALUES (
      ...
      enriched->>'provenance', enriched->>'condition', 'active', false, v_platform
    ) RETURNING id INTO new_id;
