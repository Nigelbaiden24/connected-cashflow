...
BEGIN
  ...
  -- Never invent a generic stock thumbnail during promotion. If AI generation fails,
  -- keep the image empty so the frontend can show an icon rather than a misleading generic image.
  IF v_image ~* 'images\.unsplash\.com|source\.unsplash\.com|picsum\.photos|placehold\.co|placeholder' THEN
    v_image := NULL;
  END IF;

  v_gallery := CASE WHEN v_image IS NULL THEN ARRAY[]::text[]
                    WHEN jsonb_typeof(enriched->'gallery_images') = 'array'
                    THEN ARRAY(SELECT jsonb_array_elements_text(enriched->'gallery_images') WHERE jsonb_array_elements_text ~* '^https?://' AND jsonb_array_elements_text !~* 'images\.unsplash\.com|source\.unsplash\.com|picsum\.photos|placehold\.co|placeholder')
                    WHEN jsonb_typeof(enriched->'images') = 'array'
                    THEN ARRAY(SELECT jsonb_array_elements_text(enriched->'images') WHERE jsonb_array_elements_text ~* '^https?://' AND jsonb_array_elements_text !~* 'images\.unsplash\.com|source\.unsplash\.com|picsum\.photos|placehold\.co|placeholder')
